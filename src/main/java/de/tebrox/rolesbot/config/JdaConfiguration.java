...
public class JdaConfiguration {
    ...
    public JDA jda(
        ...
        log.info("[RolesBot] JDA ready. Logged in as: {}", jda.getSelfUser().getAsTag());

        configManager.reconcile(jda);

        jda.updateCommands().addCommands(
                Commands.slash("rolesetup", "Erstellt oder editiert das Rollenpanel")
        ...
        log.info("[RolesBot] Slash commands registered. Active guilds: {}", jda.getGuilds().size());
        return jda;
    }
    ...
    public void onContextClosed() {
        if (jda != null) {
            log.info("[RolesBot] Shutting down JDA gracefully...");
            jda.shutdown();
        }
    }
}
